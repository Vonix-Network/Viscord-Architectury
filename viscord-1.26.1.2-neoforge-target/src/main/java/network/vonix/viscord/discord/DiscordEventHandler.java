    ...
    public static void registerCommands(CommandDispatcher<CommandSourceStack> dispatcher) {
        ...
        dispatcher.register(
                Commands.literal("discord")
                        .requires(source -> source.permissions().hasPermission(new net.minecraft.server.permissions.Permission.HasCommandLevel(net.minecraft.server.permissions.PermissionLevel.ALL)))
                        .executes(context -> {
                            ...
                            if (invite == null || invite.isEmpty()) {
                                ...
                                MutableComponent clickable = Component
                                        ...
                                        .withStyle(style -> style
                                                .withClickEvent(new ClickEvent.OpenUrl(java.net.URI.create(invite)))
                                                .withUnderlined(true)
        ...
        dispatcher.register(
                Commands.literal("viscord")
                        .then(Commands.literal("reload")
                                .requires(source -> source.permissions().hasPermission(new net.minecraft.server.permissions.Permission.HasCommandLevel(net.minecraft.server.permissions.PermissionLevel.ADMINS)))
                                .executes(context -> {
                                    context.getSource().sendSuccess(() -> Component.literal(
                                    ...
                                    Viscord.ASYNC_EXECUTOR.submit(() -> {
                                        try {
                                            ...
                                            // Reload config
                                            Path configPath = Viscord.getConfigDirectory();
                                            ...
                                            if (ViscordConfigToml.General.ENABLED.get()) {
                                                ...
                                                mcServer.execute(() -> source.sendSuccess(
                                                        () -> Component.literal("§aViscord reloaded successfully!"), false));
                                            } else {
                                                mcServer.execute(() -> source.sendSuccess(
                                                        () -> Component.literal("§eViscord is disabled in config."), false));
                                            }
                                        } catch (Exception e) {
                                            mcServer.execute(() -> source.sendFailure(
                                                    Component.literal("§cFailed to reload: " + e.getMessage())));
                                        ...
                                        .executes(context -> {
                                            context.getSource().sendSuccess(() -> Component.literal(
                                                    "§6§l=== Viscord Discord Commands ===\n" +
                                                            ...
                                                            "§b/discord messages§7 - Toggle cross-server messages\n" +
                                                            "§b/discord events§7 - Toggle cross-server events\n" +
